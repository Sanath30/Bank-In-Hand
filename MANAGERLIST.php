...
<head>
  ...
  if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($managerid == $adminID || $managerid == ($adminID - 1) || $managerid == ($adminID - 2)) {
      $query = "SELECT * FROM manager WHERE mn_id='$managerid'";
...
<body background="fim.png">
  ...
  <CENTER><button class="box boxes">
      ...
      <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
        <p>Manager ID<input style="float: right; background: transparent; background: transparent;" type="text"
            id="managerid" name="managerid" required></p>
        <br>
        <p>Manager Name<input style="float: right; background: transparent;" type="text" id="name" name="name"
